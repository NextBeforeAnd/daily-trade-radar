#!/usr/bin/env python3
"""Render a privacy-scrubbed Word report from validated radar JSON."""
...
DEFAULT_TEMPLATE = ASSETS_DIR / "radar-template.docx"
...
def build_report(data: dict, template_path: Path) -> Document:
    ...
    review = sum(item.get("disposition") in {"possible_update", "review_required"} for item in matches)
    if english:
